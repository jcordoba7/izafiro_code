...
public class Soldado extends Thread{
    ...
    private String agua;//Desactiva el sonido de agua de la ventana (si hay fuentes)
    private ArrayList<Soldado> otrosSoldados;
    
    private static boolean moverSoldado = true;
        
    
    public Soldado(JLabel imagen,JLabel tahi,Point[] coordenadas,int speed,int sentido,
                   Obstaculo oAba, Obstaculo oArr, Obstaculo oIzq, Obstaculo oDer, int puntoInicio, 
                   int[] vec, SenalVentana sv,String ac,ArrayList<Soldado> oS, int vL)
    {
      this.imagen = imagen;
      this.tahi = tahi;
    ...
    public int hallarLimiteIzq()
    {
       ...
       while(!limite)
       {
         ...
         {
           if(obstaculosIzquierda.getLineas().get(i).getX2() == finalIzq 
              && !(imagen.getY() > obstaculosIzquierda.getLineas().get(i).getY2()
                  || imagen.getY()+imagen.getHeight() < obstaculosIzquierda.getLineas().get(i).getY1()))     
           {
             limite = true;
           }
         }         
    ...
    {
       int finalDer = imagen.getX()+imagen.getWidth();
       ...
       {            
         for(int i=0; i<obstaculosDerecha.getLineas().size(); i++)
         {
           if(obstaculosDerecha.getLineas().get(i).getX1() == finalDer
              && !(imagen.getY() > obstaculosDerecha.getLineas().get(i).getY2()
                  || imagen.getY()+imagen.getHeight() < obstaculosDerecha.getLineas().get(i).getY1()))
           {
             limite = true;
           }
         }
    ...
    public int hallarLimiteArr()
    {
       ...
       while(!limite)
       {
         ...
         { 
           if(obstaculosArriba.getLineas().get(i).getY2() == finalArr
              && !(obstaculosArriba.getLineas().get(i).getX2() < imagen.getX()
                   || imagen.getX()+imagen.getWidth() < obstaculosArriba.getLineas().get(i).getX1()))
           {
             limite = true;
           }
         }
    ...
    {
       int finalAba = imagen.getY()+imagen.getHeight();
    ...
    { 
      ...
      {   
        ...
        if(otrosSoldados.get(i).getAgua() != null){Sonidos.getInstance().detener(otrosSoldados.get(i).getAgua());}
      }
